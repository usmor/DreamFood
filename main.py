...
def game_to_send(name=None):
    ...
    index = random.sample(range(501), 10)

    # чтобы было интереснее перемешиваем ответы
    for i in index:
        a = random.randint(0, 1)
        print()
        if a == 1:
            questions.append(
                df.iloc[i][['initial sentence', 'true sentence', 'generated text']].tolist())
        elif a == 0:
            questions.append(
                df.iloc[i][['initial sentence', 'generated text', 'true sentence']].tolist())

        # в right_answers записываем вопросы и правильные ответы
        right_answers.append(df.iloc[i][['initial sentence', 'true sentence']].tolist())
    return render_template('game.html', name=name, result=questions)


# при вызове game_count() происходит переход на страницу с ответами
@app.route('/result')
